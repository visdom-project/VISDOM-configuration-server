...
router.post("/:visId", async (req, res) => {
    ...
    if (!config || !name){
        return res.status(400).json({error: "Not found parameters"});
    }
    ...
    if (!vis) {
        try {
            ...
            console.log(err);
            return res.status(400).json({error: "bad microfrontend name"});
        }
    }
    
    if (vis.configurations.filter(config => config.name === name).length !== 0) {
        return res.status(400).json({error: "duplicate configuration name"});
    }
...
router.get("/:visId/", async (req, res) => {
    ...
    if (config.length === 0){
        return res.status(404).json({error: "config not found with name"});
    }
